...
class node:
    ...
    def __init__(self,fea=-1,value=None,results=None,right=None,left=None):
        ...
        self.result=results  #存储叶节点所属的类别
        self.right=right    #右子叶
        self.left=left     #左子叶
...
def split_tree(data,fea,value):
    """根据特征fea中的value将数据集data划分成左右子树
    input:data(list):数据集
          fea(int):待划分特征的索引
          value(float):待划分特征的具体值
...
def build_tree(data):
    ...
    for fea in range(0,feature_num):
        ...
        #2.2  针对每一个可能的值，尝试将数据集划分，并计算Gini指数
        for value in feature_values.keys():             #遍历该属性的所有切分点
            #2.2.1  根据fea特征的值value将数据集划分为左右子树
